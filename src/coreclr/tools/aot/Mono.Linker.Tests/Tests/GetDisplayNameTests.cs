        ...
        public void TestGetDisplayName(IMemberDefinition member, CustomAttribute customAttribute)
        {
            ...
            switch (member.MetadataToken.TokenType)
            {
                ...
                case TokenType.TypeDef:
                    Assert.Equal(expectedDisplayName, ((TypeReference)member).GetDisplayName());
                    break;
                case TokenType.MemberRef:
                case TokenType.Method:
                    Assert.Equal(expectedDisplayName, ((MethodReference)member).GetDisplayName());
                    break;
                case TokenType.Field:
                    Assert.Equal(expectedDisplayName, ((FieldReference)member).GetDisplayName());
                    break;
                default:
                    throw new NotImplementedException();
            }
...
public class GetDisplayNameTestsGlobalScope
{
    ...
    {
        var expectedDisplayName = (string)customAttribute.ConstructorArguments[0].Value;
        Assert.Equal(expectedDisplayName, ((MemberReference)member).GetDisplayName());
    }
